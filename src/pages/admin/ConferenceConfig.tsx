...
export default function ConferenceConfig() {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      orderDeadline: conference.orderDeadline,
  ...
  const handleSlugChange = (value: string) => {
    setSlugEdited(true)
    setForm((f) => ({ ...f, slug: value.replace(/\s+/g, '_') }))
  }

  const handleSave = async () => {
    if (!form.name.trim() || !form.slug.trim() || !form.startDate || !form.endDate || !form.orderDeadline) {
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#037EF3]/20 focus:border-[#037EF3] font-mono"
            />
            <p className="text-xs text-gray-400 mt-1">
              shop.aiesec.com.br/<span className="text-[#037EF3]">{form.slug || '...'}</span>
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Prazo pedidos *</label>
              <input
                type="date"
          ...
          <button
            ...
            onClick={handleSave}
            disabled={saving}
            className="w-full py-2.5 rounded-lg text-sm font-medium bg-[#037EF3] text-white hover:bg-[#0256B0] transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {saving && (
